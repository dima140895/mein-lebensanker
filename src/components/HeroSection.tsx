    ...
    <section className="relative min-h-[55vh] sm:min-h-[70vh] md:min-h-[85vh] flex items-center">
      <div className="container relative mx-auto px-6 py-4 pt-16 sm:px-4 sm:pt-10 sm:py-10 md:py-20">
        <div className="grid lg:grid-cols-2 gap-4 sm:gap-8 md:gap-12 items-center">
          <div key={`hero-content-${language}`} className="max-w-xl">
            {/* Badge */}
            <motion.div
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.1 }}
              className="mb-3 sm:mb-4 md:mb-6 inline-flex items-center gap-2 rounded-full border border-sage/20 bg-white/80 backdrop-blur-sm px-3 sm:px-3 md:px-4 py-1.5 sm:py-1.5 md:py-2 text-xs sm:text-xs md:text-sm font-medium text-sage-dark shadow-sm"
            >
            ...
            <motion.h1
              ...
              className="font-serif text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight text-foreground"
            >
              {t('hero.tagline')}
            </motion.h1>
            ...
            <motion.p
              ...
              className="mt-2 sm:mt-3 md:mt-5 font-serif text-lg sm:text-xl md:text-xl lg:text-2xl text-sage-dark"
            >
              {t('hero.subtitle')}
            </motion.p>
            ...
            <motion.p
              ...
              className="mt-2 sm:mt-3 md:mt-5 text-sm sm:text-base md:text-lg leading-relaxed text-muted-foreground"
            >
              {t('hero.description')}
            </motion.p>
            ...
            <motion.div
              ...
              className="mt-5 sm:mt-6 md:mt-8 flex flex-col sm:flex-row items-stretch sm:items-start gap-3 sm:gap-3 md:gap-4 px-6 sm:px-0"
            >
              <button
