      ...
      <div className="container relative mx-auto px-4 py-4 sm:py-10 md:py-20">
        <div className="grid lg:grid-cols-2 gap-4 sm:gap-8 md:gap-12 items-center">
          ...
          >
            ...
            <div className="mb-2 sm:mb-4 md:mb-6 inline-flex items-center gap-1.5 sm:gap-2 rounded-full border border-sage/20 bg-white/80 backdrop-blur-sm px-2 sm:px-3 md:px-4 py-0.5 sm:py-1.5 md:py-2 text-[10px] sm:text-xs md:text-sm font-medium text-sage-dark shadow-sm">
              <span className="relative flex h-1 sm:h-1.5 md:h-2 w-1 sm:w-1.5 md:w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-sage opacity-75"></span>
                <span className="relative inline-flex rounded-full h-1 sm:h-1.5 md:h-2 w-1 sm:w-1.5 md:w-2 bg-sage"></span>
              </span>
              <span className="line-clamp-1">{t('disclaimer.short')}</span>
            </div>

            {/* Main heading - responsive sizing */}
            <h1 className="font-serif text-xl sm:text-2xl md:text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight text-foreground">
              {t('hero.tagline')}
            </h1>

            {/* Subtitle - responsive */}
            <p className="mt-1 sm:mt-3 md:mt-5 font-serif text-sm sm:text-lg md:text-xl lg:text-2xl text-sage-dark">
              {t('hero.subtitle')}
            </p>

            {/* Description - responsive */}
            <p className="mt-1 sm:mt-3 md:mt-5 text-xs sm:text-sm md:text-lg leading-relaxed text-muted-foreground">
              {t('hero.description')}
            </p>

            {/* CTA Buttons - full width on mobile */}
            <div className="mt-3 sm:mt-6 md:mt-8 flex flex-col sm:flex-row items-stretch sm:items-start gap-2 sm:gap-3 md:gap-4">
              <button
                onClick={() => navigate('/dashboard')}
                className="group relative inline-flex items-center justify-center gap-1.5 sm:gap-2 rounded-full bg-primary px-4 sm:px-6 md:px-7 py-2 sm:py-3 md:py-3.5 text-xs sm:text-sm md:text-base font-medium text-primary-foreground shadow-lg transition-all hover:shadow-xl active:scale-[0.98]"
              >
                ...
                <ArrowRight className="relative h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5 transition-transform group-hover:translate-x-1" />
              </button>
              
              <button
                onClick={() => navigate('/mehr-erfahren')}
                className="inline-flex items-center justify-center gap-1.5 sm:gap-2 rounded-full border border-border bg-white/70 backdrop-blur-sm px-4 sm:px-6 md:px-7 py-2 sm:py-3 md:py-3.5 text-xs sm:text-sm md:text-base font-medium text-foreground transition-all hover:bg-white hover:shadow-md"
              >
                {t('hero.learnMore')}
              </button>
            </div>
